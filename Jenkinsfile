        ...
        stage('Deploy for production') {
            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'servercredential', keyFileVariable: 'KEY', passphraseVariable: 'PASSPHRASE', usernameVariable: 'USERNAME'), string(credentialsId: 'myserverip', variable: 'IP')]) {
                    echo 'Sync...'
                    sh 'rsync -avrt --delete --exclude \'.git\' --rsh=\'ssh -p 22\' ./ $USERNAME@$IP:/$USERNAME/react-marketo-id.rizalibnu.com/'

                    echo 'Build Docker...'
                    sh '''
                    ssh -t -i "$KEY" "$USERNAME"@"$IP" -p 22 "cd /$USERNAME/react-marketo-id.rizalibnu.com && sh start.sh"
                    '''
                }
            }
        }
